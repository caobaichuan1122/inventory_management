...
def modify_fix_tr_product(request,id):
    if request.method == 'POST':
        fix_id = request.POST.get('fixed_id')
        fix_product_id = request.POST.get('product_id')
        fix_product_name = request.POST.get('product_Name')
        product_Issue= request.POST.get('product_Issue')
        product_state = request.POST.get('FixedState')
        product_record = request.POST.get('editor1')
        models.fix_tr_report.objects.filter(id=id).update(tr_product_id=fix_product_id,tr_product_name=fix_product_name,tr_product_description=product_Issue,fix_state = product_state, fixed_detail = product_record)
        return redirect('/TrRepairList/')
    else:
        return render(request, '/TrRepairList/',locals())
...
def modify_tr_product(request,id):
    if request.method == 'POST':
        ...
        Product_Cost = request.POST.get('Product_Cost')
        Product_time = request.POST.get('Product_time')
        models.Trproduct.objects.filter(id=id).update(tr_product_id = product_id, tr_product_name = product_name, tr_product_price = Product_Price,
                                                      tr_product_cost = Product_Cost, tr_product_time = Product_time)
        return redirect('/TrProductList/')
    else:
...
def modify_tp_product(request,id):
    if request.method == 'POST':
        ...
        Product_Cost = request.POST.get('Product_Cost')
        Product_time = request.POST.get('Product_time')
        models.Tproduct.objects.filter(id=id).update(tp_product_id = product_id, tp_product_name = product_name, tp_product_price = Product_Price,
                                                     tp_product_cost = Product_Cost, tp_product_time = Product_time)
...
def modify_pr_product(request,id):
    if request.method == 'POST':
        ...
        Product_Cost = request.POST.get('Product_Cost')
        Product_time = request.POST.get('Product_time')
        models.Prproduct.objects.filter(id=id).update(pr_product_id = product_id, pr_product_name = product_name, pr_product_price = Product_Price,
                                                      pr_product_cost = Product_Cost, pr_product_time = Product_time)
...
def generatePDF(request):
    ...
    if request.method == "POST":
        ...
        customer_info = customer.objects.filter(customer_id=customerid).values()
        product_list = Trproduct.objects.filter(id__in=product_id).values()
        sum_quantity = str(len(product_id))
        return render(request,'login/generate_pdf.html',context={'company_id':company_id,'Delivery_No':Delivery_No,'Delivery_Term':Delivery_Term,
                                                                 'product_list':product_list,'customer_info':customer_info,
...
def generatePDF_tp(request):
    ...
    if request.method == "POST":
        ...
        return render(request,'login/generate_pdf.html',context={'company_id':company_id,'Delivery_No':Delivery_No,
                                                                 'Delivery_Term':Delivery_Term,'product_list':product_list,'customer_info':customer_info,
...
def tr_order(request):
    ...
    return render(request,'login/tr_order_list.html',context={"tr_list":tr_list})

def tp_order(request):
    tp_list = tp_order_list.objects.all()
    return render(request,'login/tp_order_list.html',context={"tp_list":tp_list})

def pr_order(request):
    pr_list = tr_order_list.objects.all()
    return render(request,'login/pr_order_list.html',context={"pr_list":pr_list})

def repair_record_edit(request,id):
    FixTrDatabase = fix_tr_report.objects.all()
    company_id = "1"
    if id:
        item_id = id
        Value_change = fix_tr_report.objects.filter(id=item_id).values()
        return render(request,"login/RepairEdit.html",context={"Value_change":Value_change[0],"item_id":item_id,"company_id":company_id})
    else:
        return render(request, 'login/TRrepairList.html',context={'FixTrDatabase': FixTrDatabase})

def repair_record_edit_tp(request,id):
    FixTpDatabase = fix_tp_report.objects.all()
    company_id = "2"
    if id:
        item_id = id
        Value_change = fix_tp_report.objects.filter(id=item_id).values()
        return render(request,"login/RepairEdit.html",context={"Value_change":Value_change[0],"item_id":item_id,"company_id":company_id})
    else:
        return render(request, 'login/TPrepairList.html',context={'FixTpDatabase': FixTpDatabase})

def repair_tr_report(request,id):
    database_value = fix_tr_report.objects.filter(id=id).values()
    company_id = "1"
    return render(request,'login/repair_report.html',context={"database_value":database_value[0],"company_id":company_id})


def repair_tp_report(request,id):
